ic.test <- function(obj,TP=1,s2=1, df.error=Inf, 
...
### checks
    ...
    if (!length(b.unrestr)==length(b.restr))
        stop("b.unrestr and b.restr are of different length.")
    if (s2<=0) stop("test not possible with non-positive s2.")
    if (df.error==Inf) cov <- cov*s2
    if (TP==11){ 
         if (!is.null(ci0.11)) 
            if (any(!ci0.11==0)) stop("ci0.11 must be a vector of 0es.")
         if (is.vector(ui0.11)) ui0.11 <- matrix(ui0.11, 1, length(ui0.11))
         if (is.null(ci0.11)) ci0.11 <- rep(0,nrow(ui0.11))
         if (!ncol(ui0.11)==g) stop(paste("ui0.11 must have", g, "columns."))
         if (!nrow(ui0.11)==length(ci0.11)) stop(paste("mismatch between ui0.11 and ci0.11"))
    }
      hilf <- RREF(t(ui), tol=tol)
      ## show linear independent subset of rows of ui
      if (hilf$rank < nrow(ui)) 
        stop (paste("Matrix ui must have full row-rank (choose e.g. rows", paste(hilf$pivot,collapse=" "), ")."))
...
if (TP==11){
       ...
       ui.extra <- ui0.11%*%(diag(rep(1,g))-t(uiw)%*%solve(uiw%*%t(uiw),uiw))
       if (all(abs(ui.extra)<tol)) TP <- 1
       else {
             ### eliminate 0 rows
             ui.extra <- ui.extra[!rowSums(abs(ui.extra)<tol)==g,]
             if (is.vector(ui.extra)) ui.extra <- matrix(ui.extra,1,length(ui.extra))
             else {   
...
if (TP==3 & obj$meq>0) stop("TP3 not applicable in case of equality restrictions.")
if (!TP==3)
if (!all(uiw%*%b.restr-ci>=-1/1000000 )) 
...
if (TP %in% c(1,2,11)) {
   ...
   if (is.null(wt)) {
       if (obj$meq==0) wt.bar <- ic.weights(uiw%*%cov%*%t(uiw), ...) 
       else {
           wt.bar <- ic.weights(solve(solve(uiw%*%cov%*%t(uiw))[-(1:obj$meq),-(1:obj$meq)]), ...)
            }
       }
       else wt.bar <- wt
}
...
if (TP==1){
   b.eqrestr=solve.QP(Dmat=solve(cov),dvec=solve(cov,b.unrestr),
                          Amat=t(uiw),bvec=ci,meq=nrow(ui))$solution
   b.eqrestr[abs(b.eqrestr) < tol] <- 0
   names(b.eqrestr) <- names(b.restr)
   T <- t(b.restr-b.eqrestr)%*%solve(cov,b.restr-b.eqrestr)
   if (is.null(df)) df.bar <- (nrow(ui)-obj$meq):0 else df.bar <- df
   if (df.error==Inf) p.value <- 1-pchibar(T,df=df.bar,wt=wt.bar)
      else { T <- T/(df.error*s2+T)  ### T excluding s2, i.e. not vcov but cov.unscaled
             p.value <- 1-pbetabar(T,df1=df.bar/2,df2=df.error/2,wt=wt.bar)
       }
   }
...
if (TP==11){
   b0 <- ic.est(b.unrestr,ui=ui0.11,ci=ci0.11,
          Sigma=cov,meq=length(ci0.11))$b.restr
   ...
   ### active
   if (is.null(df)) df.bar <- (length(b.restr)-obj$meq):(length(b.restr)-nrow(uiw)) 
      else df.bar <- df
   if (df.error==Inf) p.value <- 1-pchibar(T,df=df.bar,wt=wt.bar)
      else { T <- T/(df.error*s2+T)  ### T excluding s2, i.e. not vcov but cov.unscaled
             p.value <- 1-pbetabar(T,df1=df.bar/2,df2=df.error/2,wt=wt.bar)
...
### restricted vs. not
if (TP==2){
   T <- t(b.unrestr-b.restr)%*%solve(cov,b.unrestr-b.restr)
   if (is.null(df)) df.bar <- obj$meq:nrow(ui) 
      else df.bar <- df
   if (df.error==Inf) p.value <- 1-pchibar(T,df=df.bar,wt=wt.bar)
      else { T <- T/(df.error*s2+T)   ### T excluding s2, i.e. not vcov but cov.unscaled
             p.value <- 1-pbetabar(T,df1=df.bar/2,df2=df.error/2,wt=wt.bar)
...
if (TP==3){
   T <- min((uiw%*%b.unrestr-ci)/sqrt(diag(uiw%*%cov%*%t(uiw))))
   if (df.error==Inf) p.value <- 1-pnorm(T)
      else { T <- T/sqrt(s2)  ### T excluding s2, i.e. not vcov but cov.unscaled
             p.value <- 1-pt(T,df.error)
       }
   }
### output results
aus <- list(TP=TP, b.unrestr=b.unrestr, b.restr=b.restr, ui=ui,ci=ci, 
      restr.index=index, meq=obj$meq, iact = obj$iact, ui.extra=ui.extra,
      b.eqrestr=b.eqrestr, b.extra.restr=b0, T=T, p.value=p.value, s2=s2, cov=cov, 
      df.error=df.error, df.bar=df.bar, wt.bar=wt.bar)
class(aus) <- "ict"
aus
}
